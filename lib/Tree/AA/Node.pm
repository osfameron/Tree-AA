...
extends 'Tree::AA';
with 'MooX::Role::But';
use Types::Standard qw( Int InstanceOf CodeRef );
use List::Util 'min';
...
sub leftmost {
    my $self = shift;
    return $self->left->level ? $self->left->leftmost : $self;
}
...
sub delete {
    ...
    if (my $cmp = $cmp_ref->($key, $self->key)) {
        ...
        # $tree = $self->traverse->go($dir)->call(delete => $key)->focus;
        $tree = $self->but(
            $dir => $self->$dir->delete($key)
        );
    }
    else {
        if ($self->leaf) {
